...
https://nightlies.apache.org/flink/flink-docs-release-1.14/zh/docs/connectors/table/kafka/
https://nightlies.apache.org/flink/flink-docs-release-1.14/zh/docs/dev/python/table_api_tutorial/

涉及知识点： 1.建表(kafka源) 2.注册函数(训练模型的函数) 3.执行所注册的函数,将执行后的结果呈现(推理结果)
"""
...
# kafka topic
source_topic = "handwritten_digit"
sink_topic = "digit_predict"
...
class Model(ScalarFunction):
    ...
    def load_model(self):
        ...
        try:
            ...
            clf = clf or SGDClassifier(alpha=0.01, loss='log_loss', penalty='l1')
